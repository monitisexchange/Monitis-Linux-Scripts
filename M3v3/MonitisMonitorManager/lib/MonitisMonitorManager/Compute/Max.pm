...
# takes the maximum value out of X iterations
...
sub compute {
	...
	# some sanity testing
	if ( $code <= 0 ) {
		croak "Iteration number X  - <max>X</max> - must be bigger than 0";
	}
	...
	foreach my $metric_name (keys %{$results}) {
		...
		if ( !defined(&retrieve_value($agent_name, $monitor_name, $metric_name)) ) {
			...
			$iteration++;
			store_value($agent_name, $monitor_name, $metric_name, max($old_data, $new_data), $iteration);
			...
			if ( $iteration % $code == 0 ) {
				...
				# it's not time to return a max value, wait for next iteration
				push @keys_to_remove, $metric_name;
			}
		}
	}
...
sub store_value {
	...
	$max_hash{$agent_name . "_" . $monitor_name . "_" . $metric_name} = { "data" => $data, "iteration" => $iteration };
}

sub retrieve_value {
	my ($agent_name, $monitor_name, $metric_name) = @_;
	return $max_hash{$agent_name . "_" . $monitor_name . "_" . $metric_name};
}

sub delete_value {
	my ($agent_name, $monitor_name, $metric_name) = @_;
	delete $max_hash{$agent_name . "_" . $monitor_name . "_" . $metric_name};
}
